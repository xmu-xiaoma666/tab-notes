...
现在可以直接右键 Chrome 顶部的任意 Tab，点击「修改页签备注」，给网页添加：
...
以下配图均为 Chrome 中渲染的脱敏 PNG 截图，使用 Rednote 虚构页面和公开信息，不包含真实账号、内部网址、浏览记录或工作数据。
...
图片说明：右键选择网页、填写备注、保存后标题立即可辨。

### 图 3：右键快捷入口

![Tab 右键菜单中的修改页签备注](store-assets/guide-right-click.png)

图片说明：不需要先打开插件，直接右键目标 Tab 修改。
