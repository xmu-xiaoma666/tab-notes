...
> 给大量相似 Chrome 网页添加短标题、分类、颜色和详细备注。状态：v0.4.1；更新于 2026-08-18。
...
- 可按网页地址前缀设置默认短标题、分类、颜色和详细备注，单个网页仍可独立修改。
...
在侧边栏的“网址前缀规则”区域可以添加一条默认规则：

1. 输入网页地址前缀，例如 `https://example.com/project/`。
2. 填写这个前缀下网页默认使用的标签短标题。
3. 按需选择默认分类、默认颜色，并填写默认详细备注。
4. 点击“保存规则”。

匹配该前缀、且尚未保存单页备注的网页会立即使用这组默认值。短标题、分类、颜色和详细备注都可以在当前网页编辑器中直接修改；保存后会成为该网页自己的备注，并完整覆盖前缀默认值。

- 已有的逐页备注始终优先，不会被前缀规则覆盖。
- 删除单页备注后，如果网址仍匹配规则，会恢复前缀默认值。
- 多条规则同时匹配时，最长、最具体的网址前缀优先。
- 修改或删除规则只影响仍在使用默认值的网页，不会删除已经保存的单页备注。
...
v0.4.0 增加“网址前缀规则”：可以为一组网页设置默认短标题和默认详细备注；原有逐页备注完整保留并优先于默认规则。

v0.4.1 为网址前缀规则补充默认分类和默认颜色；旧规则会自动使用空分类和蓝色，不影响已有数据。
